...
public class AK_Male_Cannabis_Plant extends _Male_Cannabis_Plant {
    public AK_Male_Cannabis_Plant(Settings settings) {super(settings);}
    ...
    public void pollinate(BlockState state, ServerWorld world, BlockPos pos, Random random, Float f)
    {
        ...
        if (random.nextInt((int) (25.0F / f) + 1) == 0) {
            ...
            {
                if (world.getBlockState(positions).getBlock() == ModBlocks.AFG_FEMALE_CANNABIS_PLANT || world.getBlockState(positions).getBlock() == ModBlocks.HK_FEMALE_CANNABIS_PLANT)
                {
                    _Female_Cannabis_Plant fem_plant = (_Female_Cannabis_Plant) world.getBlockState(positions).getBlock();
                    fem_plant.getPollinated(world, positions, ModBlocks.AK_MALE_CANNABIS_PLANT.getDefaultState());
                }
